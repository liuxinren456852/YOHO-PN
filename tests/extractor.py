...
class extractor_PartI():
    ...
    def Extract(self,dataset):
        ...
        self._load_model()
        self.network.eval()
        PN_input_dir=f'{self.cfg.output_cache_fn}/Testset/{dataset.name}/PN_Input_Group_feature'
        ...
        for pc_id in tqdm(dataset.pc_ids):
            ...
            Input_feature=np.load(f'{PN_input_dir}/{pc_id}.npy') #5000*32*60
            output_feature=[]
            bi=0
...
class extractor_PartII():
    ...
    def batch_create(self,feats0_pn,feats1_pn,feats0_yomo,feats1_yomo,index_pre,start,end):
        #attention: here feats0->feats1_in_batch for it is afterrot
        feats0_pn=torch.from_numpy(feats0_pn[start:end,:,:].astype(np.float32))
        feats1_pn=torch.from_numpy(feats1_pn[start:end,:,:].astype(np.float32))
        ...
        return {
                'before_eqv0':feats1_pn,#exchanged
                'before_eqv1':feats0_pn,
    ...
    def PartII_R_pre(self,dataset):
        ...
        if dataset.name[0:4]=='3dLo':
            ...
            datasetname=dataset.name
        PN_dir=f'{self.cfg.output_cache_fn}/Testset/{datasetname}/PN_Input_Group_feature'
        ...
        for pair in tqdm(dataset.pair_ids):
            ...
            feats0_pn=np.load(f'{PN_dir}/{id0}.npy')[pps[:,0],:,:] #pps*32*60
            feats1_pn=np.load(f'{PN_dir}/{id1}.npy')[pps[:,1],:,:] #pps*32*60
            ...
            bi=0
            Rs=[]
            while(bi*self.cfg.test_batch_size<feats0_pn.shape[0]):
                start=bi*self.cfg.test_batch_size
                end=(bi+1)*self.cfg.test_batch_size
                batch=self.batch_create(feats0_pn,feats1_pn,feats0_yomo,feats1_yomo,Index_pre,start,end)
                batch=to_cuda(batch)
                with torch.no_grad():
                    batch_output=self.network(batch)
                bi+=1
