...
def R2DR_id(R):
    ...
    min_diff=180
    best_id=0
    for R_id in range(Rgroup.shape[0]):
        R_diff=compute_R_diff(Rgroup[R_id],R)
        if R_diff<min_diff:
            min_diff=R_diff
            best_id=R_id
    return best_id
...
class Evaluator_PartI:
    ...
    def eval(self):
        ...
        Mean_Registration_Recall,c_flags,c_errors=RR_cal.benchmark(self.cfg,datasets,self.max_iter,yoho_sign='YOMO_C')
...
class Evaluator_PartII:
    ...
    def eval(self):
        ...
        #RR
        datasetname=datasets['wholesetname']
        Mean_Registration_Recall,c_flags,c_errors=RR_cal.benchmark(self.cfg,datasets,self.max_iter,yoho_sign='YOMO_O')
        #print and save:
        print(FMRS)
        msg=f'{datasetname}-{self.cfg.descriptor}-{self.cfg.extractor}-{self.cfg.matcher}-{self.cfg.estimator}-{self.max_iter}iterations\n'
        msg+=f'correct ratio avg {np.mean(all_pair_fmrs):.5f}\n' \
