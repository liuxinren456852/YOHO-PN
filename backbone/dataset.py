...
import numpy as np
from utils.utils import transform_points,random_rotation_matrix
...
class ThreeDMatchDataset(Data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        anc_ind=np.random.choice(range(anc_local_patch.shape[0]-1),size=1023,replace=False)
        anc_ind=np.concatenate([anc_ind,np.array([anc_local_patch.shape[0]-1])])
        pos_ind=np.random.choice(range(pos_local_patch.shape[0]-1),size=1023,replace=False)
        pos_ind=np.concatenate([pos_ind,np.array([pos_local_patch.shape[0]-1])])
        anc_local_patch=anc_local_patch[anc_ind]
        pos_local_patch=pos_local_patch[pos_ind]


        rotate = patches[:2, :].reshape(12)[:9].reshape(3, 3)  
        shift = patches[:2, :].reshape(12)[9:]                 
        T=np.concatenate([rotate,shift[:,None]],axis=1)
        #align
        anc_local_patch = transform_points(anc_local_patch,T)
        #random rotate
        rand_R = random_rotation_matrix(180)
        anc_local_patch = transform_points(anc_local_patch,rand_R)
        pos_local_patch = transform_points(pos_local_patch,rand_R)
        #difference
        rotate = random_rotation_matrix(50)
        anc_local_patch = transform_points(anc_local_patch,rotate)
        shift=np.zeros([3])
